    ...
    methods: {
      ...
      reInitPages(){
        ...
        children.forEach( function( child, index){
            ...
            if (index === 0) { addClass(child.$el, 'is-active') }
        })
      ...
      doAnimate(towards, options){
        if (this.$children.length === 0 || !options && this.$children.length < 2) return;
      ...
      goto(newIndex){
        if(newIndex < this.index){
          this.doAnimate('goto', {
            newIndex, prevPage: this.pages[newIndex]
          })
        } else {
          this.doAnimate('goto', {
            newIndex, nextPage: this.pages[newIndex]
          })
        }
      },
      ...
      doOnTouchStart(event){
        ...
        if (this.continuous && this.pageLength > 1) {
          if (!prevPage) {
